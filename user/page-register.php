...
if( !empty($_POST['register_reg']) ) {
	...
	// verification
	if(mokore_option('login_validate') && strlen($_POST['verification']) > 0 ){
...
?>
	<div id="primary" class="content-area">
		...
		<?php if(mokore_option('ex_register_open')) : ?>
		<?php if(!is_user_logged_in()){ ?>
			<div class="ex-register">
				...
				<form action="<?php echo $_SERVER["REQUEST_URI"]; ?>" method="post">  
					...
					<?php if(mokore_option('login_validate')) : ?>
		...
		<?php }else{ 
		$loadurl = mokore_option('exlogin_url') ? mokore_option('exlogin_url') : get_bloginfo('url');
		?>
